...
public static partial class PAnims
{
    ...
    {
        if (node == null) { Logger.Error("Cannot play animation on null"); return false; }
        if (!node.IsInsideTree()) { Logger.Error("Cannot play animation on node not inside tree"); return false; }
        return true;
    }
}
